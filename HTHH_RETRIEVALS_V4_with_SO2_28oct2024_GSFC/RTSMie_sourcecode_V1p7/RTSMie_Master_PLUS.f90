...
!  I/O Type structures
   ...
   TYPE(RTSMie_Outputs)    , INTENT (inout)    :: Mie_Outputs
   TYPE(RTSMie_Lin_Outputs), INTENT (inout)    :: Mie_Lin_Outputs
...
!       Fmatrix_angles   = user-defined angles, in DEGREES between [0, 180]. USED DIRECTLY.
...
!  Main output
   ...
   Mie_Outputs%Mie_dist1  = d_zero
   Mie_Outputs%Mie_dist2  = d_zero   ! Second distribution only needed for Bimodal computations
   Mie_Outputs%Mie_bulk   = d_zero
   Mie_Outputs%Mie_asymm  = d_zero

   Mie_Outputs%Mie_Fmatrix    = d_zero
   Mie_Outputs%Mie_ncoeffs    = 0
   Mie_Outputs%Mie_expcoeffs  = d_zero

   Mie_Lin_Outputs%LPSD_Mie_dist  = d_zero
   Mie_Lin_Outputs%LPSD_Mie_bulk  = d_zero ; Mie_Lin_Outputs%LRFE_Mie_bulk  = d_zero 
   Mie_Lin_Outputs%LPSD_Mie_asymm = d_zero ; Mie_Lin_Outputs%LRFE_Mie_asymm = d_zero

   Mie_Lin_Outputs%LPSD_Mie_expcoeffs = d_zero ; Mie_Lin_Outputs%LRFE_Mie_expcoeffs = d_zero
   Mie_Lin_Outputs%LPSD_Mie_Fmatrix   = d_zero ; Mie_Lin_Outputs%LRFE_Mie_Fmatrix   = d_zero
...
!  Input Proxies (All single numbers)
!  ----------------------------------

   nweights = Mie_inputs%nweights
   nblocks  = Mie_inputs%nblocks

   lambda = Mie_inputs%lambda
   n_real = Mie_inputs%n_real
   n_imag = Mie_inputs%n_imag

   do_monodisperse = Mie_inputs%do_monodisperse
   Do_Expcoeffs    = Mie_inputs%Do_Expcoeffs

   Monoradius      = Mie_inputs%Monoradius
   xparticle_limit = Mie_inputs%xparticle_limit

   R1          = Mie_inputs%R1
   R2          = Mie_inputs%R2
   FixR1R2     = Mie_inputs%FixR1R2
   R1R2_cutoff = Mie_inputs%R1R2_cutoff
   PSD_index   = Mie_inputs%PSD_index
   PSD_pars    = Mie_inputs%PSD_pars

   Do_Fmatrix       = Mie_inputs%Do_Fmatrix
   n_Fmatrix_angles = Mie_inputs%n_Fmatrix_angles

   do_LinearREF = Mie_inputs%do_LinearREF
   do_LinearPSD = Mie_inputs%do_LinearPSD
...
!  Quadrature (only need to do it once)
   ...
   IF ( Do_Expcoeffs ) THEN
      ...
      Mie_Outputs%Mie_ncoeffs = n_coeffct_angles
   END IF
   ...
   IF ( Do_Expcoeffs ) THEN
      ...
      IF ( Do_Fmatrix ) THEN
         DO angle = 1, n_Fmatrix_angles
            Fmatrix_cosines(angle) = dcos(Mie_Inputs%Fmatrix_angles(angle)*dtr)
         END DO
      ENDIF
   ELSE
      IF ( Do_Fmatrix ) THEN
         ...
         DO angle = 1, n_angles
            angle_cosines(angle) = dcos(Mie_Inputs%Fmatrix_angles(angle)*dtr)
         END DO
         do_angular_variation = .TRUE.
      END IF
   ENDIF
...
!  Bulk properties
      ...
      Mie_Outputs%MIE_BULK(1) = Qext
      Mie_Outputs%MIE_BULK(2) = Qsca
      Mie_Outputs%MIE_BULK(3) = ssalbedo
      Mie_Outputs%MIE_ASYMM   = Qasy
      if ( do_LinearRef ) then
         DO md = 1, 2
            Mie_Lin_Outputs%LRFE_MIE_BULK(1,md) = Qext_rd(md)
            Mie_Lin_Outputs%LRFE_MIE_BULK(2,md) = Qsca_rd(md)
            Mie_Lin_Outputs%LRFE_MIE_BULK(3,md) = ssalbedo_rd(md)
            Mie_Lin_Outputs%LRFE_MIE_ASYMM(md)  = Qasy_rd(md)
         END DO
      endif

!  Distribution properties. NO DERIVATIVES HERE !!!!!!!!

      Mie_Outputs%MIE_DIST1(1) = ndens
      Mie_Outputs%MIE_DIST1(2) = gxsec
      Mie_Outputs%MIE_DIST1(3) = volume
      Mie_Outputs%MIE_DIST1(4) = reff
      Mie_Outputs%MIE_DIST1(5) = veff
...
!  Bulk properties
      ...
      IF ( do_LinearPSD ) THEN
         DO kd = 1, nkderivs
            Mie_Lin_Outputs%LPSD_MIE_BULK(1,kd) = Qext_kd(kd)
            Mie_Lin_Outputs%LPSD_MIE_BULK(2,kd) = Qsca_kd(kd)
            Mie_Lin_Outputs%LPSD_MIE_BULK(3,kd) = ssalbedo_kd(kd)
            Mie_Lin_Outputs%LPSD_MIE_ASYMM(kd)  = Qasy_kd(kd)
         END DO
      END IF
...
!  Distribution properties. NO DERIVATIVES HERE !!!!!!!!
      ...
      IF ( do_LinearPSD ) THEN
         DO kd = 1, nkderivs
            Mie_Lin_Outputs%LPSD_MIE_DIST(1,kd) = ndens_d(kd)   
            Mie_Lin_Outputs%LPSD_MIE_DIST(2,kd) = gxsec_d(kd)   
            Mie_Lin_Outputs%LPSD_MIE_DIST(3,kd) = volume_d(kd)   
            Mie_Lin_Outputs%LPSD_MIE_DIST(4,kd) = reff_d(kd)   
            Mie_Lin_Outputs%LPSD_MIE_DIST(5,kd) = veff_d(kd)
         END DO
      END IF
...
!  Call to develop_d to get expansion coefficients and any necessary derivatives
   ...
   if ( do_Expcoeffs ) then
      CALL develop_d ( do_LinearRef, do_LinearPSD, max_Mie_angles, nkderivs,              & ! I
                       ...
                       Mie_Outputs%Mie_Expcoeffs, Mie_Lin_Outputs%LRFE_Mie_Expcoeffs, Mie_Lin_Outputs%LPSD_Mie_Expcoeffs )
   endif
...
!    Otherwise, just copy the already-calculated F-matrix and any necessary derivatives
   ...
   if ( do_Fmatrix ) then
     if ( do_Expcoeffs ) then
       CALL expand_d &
              ( do_LinearRef, do_LinearPSD, max_Mie_angles, nkderivs,      & ! I
                n_coeffct_angles, n_Fmatrix_angles, Fmatrix_cosines,       & ! I
                Mie_Outputs%Mie_Expcoeffs, Mie_Lin_Outputs%LRFE_Mie_Expcoeffs, Mie_Lin_Outputs%LPSD_Mie_Expcoeffs, & ! I
                Mie_Outputs%Mie_Fmatrix,   Mie_Lin_Outputs%LRFE_Mie_Fmatrix  , Mie_Lin_Outputs%LPSD_Mie_Fmatrix )    ! I/O 
     else
       Mie_Outputs%Mie_Fmatrix(1:4,1:n_Fmatrix_angles) = Local_Fmatrix(1:4,1:n_Fmatrix_angles)
       if ( do_LinearRef ) then
         Mie_Lin_Outputs%LRFE_Mie_Fmatrix(1:4,1:n_Fmatrix_angles,1:2) = Local_Fmatrix_rd(1:4,1:n_Fmatrix_angles,1:2)
       endif
       if ( do_LinearPSD ) then
         Mie_Lin_Outputs%LPSD_Mie_Fmatrix(1:4,1:n_Fmatrix_angles,1:nkderivs) = Local_Fmatrix_kd(1:4,1:n_Fmatrix_angles,1:nkderivs)
       endif
     endif
   endif
...
!  ======
  ...
  RETURN
END SUBROUTINE RTSMie_Master_plus
...
end MODULE RTSMie_Master_plus_m
