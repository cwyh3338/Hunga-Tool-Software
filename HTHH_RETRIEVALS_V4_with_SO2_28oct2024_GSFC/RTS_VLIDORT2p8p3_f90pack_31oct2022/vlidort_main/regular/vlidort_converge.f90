...
contains
      ...
      SUBROUTINE VLIDORT_CONVERGE ( AWI, &
              ...
              DO I = 1, N_OUT_STREAMS
                DO UA = 1, LOCAL_N_USERAZM
                  ...
                  DO O = 1, NSTOKES
                    ...
                    !CALL TP7G (AWI,FOURIER,UT,I,IBEAM,V,O,W,VLIDORT_Out%TS_STOKES,STOKES_F)
                  ENDDO
                ENDDO
               ENDDO
            ENDDO
          ...
          DO UT = 1, N_USER_LEVELS
            DO I = 1, N_OUT_STREAMS
              DO UA = 1, LOCAL_N_USERAZM
                V  = VZA_OFFSETS (IBEAM,I) + UA
                DO O = 1, NSTOKES
                  !CALL TP7E (AWI,FOURIER,UT,V,O,W,VLIDORT_Out%TS_STOKES,VLIDORT_SS%TS_STOKES_SS)
          ...
          DO UT = 1, N_USER_LEVELS
            DO I = 1, N_OUT_STREAMS
              DO UA = 1, LOCAL_N_USERAZM
                V  = VZA_OFFSETS(IBEAM,I) + UA
                DO O = 1, NSTOKES
                  !CALL TP7F (AWI,FOURIER,UT,V,O,VLIDORT_Out%TS_STOKES,VLIDORT_SS%TS_STOKES_DB)
...
!
      ...
      SUBROUTINE VLIDORT_CONVERGE_DOUBLET ( AWI, &
...
!
      ...
      SUBROUTINE VLIDORT_CONVERGE_OBSGEO ( AWI, &
         DO_FOCORR, DO_FOCORR_ALONE, DO_DBCORRECTION, DO_UPWELLING, DO_NO_AZIMUTH, & ! Input flags
         DO_RAYLEIGH_ONLY, DO_ALL_FOURIER, DO_DOUBLE_CONVTEST, DO_TOA_CONTRIBS,    & ! Input flags
...
!             ((DO_SSCORR_NADIR.OR.DO_SSCORR_OUTGOING).OR.DO_SS_EXTERNAL) ) THEN
        ...
        IF ( DO_FOCORR ) THEN
          DO IDIR = 1, N_DIRECTIONS
            ...
            DO UT = 1, N_USER_LEVELS
              DO O = 1, NSTOKES
                !CALL TP7E2 (AWI,FOURIER,UT,IBEAM,O,W,VLIDORT_Out%TS_STOKES,VLIDORT_SS%TS_STOKES_SS)
...
!        IF ( ((DO_SSFULL.OR.DO_DBCORRECTION).OR.DO_SS_EXTERNAL) .AND.DO_UPWELLING ) THEN
        IF ( (DO_FOCORR.OR.DO_DBCORRECTION) .AND.DO_UPWELLING ) THEN
          DO UT = 1, N_USER_LEVELS
            DO O = 1, NSTOKES
              !CALL TP7F2 (AWI,FOURIER,UT,IBEAM,O,VLIDORT_Out%TS_STOKES,VLIDORT_SS%TS_STOKES_DB)
...
!     - for direction, user optical depth
            ...
            DO IDIR = 1, N_DIRECTIONS
              ...
              DO UT = 1, N_USER_LEVELS
                DO O = 1, NSTOKES
                  !CALL TP7G2 (AWI,FOURIER,UT,UM,IBEAM,O,W,VLIDORT_Out%TS_STOKES,STOKES_F)
