...
!  Mie modules
      ...
      !USE RTSMie_Master_m
      USE RTSMie_master_bimodal_OMP_m
...
!  Special phase function plotting
!mick mod 1/9/2023 - added this section

      real(mpk)     :: del
      logical       :: Do_Normal_Mie_Calc
...
!  OpenMP tests (timing)
      ...
      real          :: e1, e2, e3, InvMod_Time, Ret_Time
      integer       :: ncores
      logical       :: do_timing
...
!mick mod 5/9/2023 - added this section
   ...
   U100 = 100
   Open(unit=U100,file='Tonga_ret_timing_OMP.log',status='replace')
...
!mick fix 8/17/2023 - added "retrieve_plumehwhm" to be able to retrieve plume HWHM
      ...
      OPEN(1,file='Toplevel_Inputs_V4_OMP.cfg', STATUS = 'old')
      read(1,*) ! OpenMP COMPUTING CONTROL
      read(1,*)ncores             ! Number of processor cores
      read(1,*)do_timing          ! Forward model timing
...
!  Begin Chi-Square Check Loop (this loop ends @ ~line 1550)
...
!  Headers
!mick mod 9/12/2022 - added tail to file name to indicate number of OpenMP threads used
...
!mick mod 10/28/2024 - added noise option #4
      ...
      elseif (NoiseOption .eq. 4) then
        do ib=1,numbands
          ...
          !define the SNR of and error quantities for this wvl subset
...
!  Special section: generate Mie output to only plot phase function
!mick mod 1/9/2023 - added this section

         Do_Normal_Mie_Calc = .true.  !For retrieval (default)
         !Do_Normal_Mie_Calc = .false. !For plotting phase function only

         if ( .not.Do_Normal_Mie_Calc ) then

           write(*,'(/1x,a)') 'Doing Mie calculation for plotting phase function only'

!  Set Mie inputs

           Mie_Inputs_Global(1:2)%lambda = 0.295_mpk !in um (near center of current
                                                   !       TROPOMI band used)

           !For F-matrix (the transformation matrix)
           !Note: # of Fmatrix angles defined based on # of coeffs required for
           !      the Mie calc at this wvl (brief preliminary run done to determine
           !      this value)
           !Mie_Inputs_Global(1:2)%do_Expcoeffs = .true.

           Mie_Inputs_Global(1:2)%do_Expcoeffs = .false.
           Mie_Inputs_Global(1:2)%do_Fmatrix   = .true.
           Mie_Inputs_Global(1:2)%n_Fmatrix_angles = 658 !max_Mie_angles
           del = 180.0_mpk/real(Mie_Inputs_Global(1)%n_Fmatrix_angles-1,mpk)
           write(*,'(/1x,a,i3)') 'n_Fmatrix_angles = ',Mie_Inputs_Global(1)%n_Fmatrix_angles
           do j = 0, Mie_Inputs_Global(1)%n_Fmatrix_angles-1
             i = j + 1
             Mie_Inputs_Global(1)%Fmatrix_angles(i) = del*real(j,mpk)
             write(*,'(1x,a,i3,a,f6.2)') 'i = ',i,'  Fmatrix_angles(i) = ',Mie_Inputs_Global(1)%Fmatrix_angles(i)
           enddo
           Mie_Inputs_Global(2)%Fmatrix_angles(1:Mie_Inputs_Global(1)%n_Fmatrix_angles) = &
           Mie_Inputs_Global(1)%Fmatrix_angles(1:Mie_Inputs_Global(1)%n_Fmatrix_angles)
           !stop 'at Fmatrix angles check'

!  Main call at chosen wavelength

           write(*,'(/1x,a,es24.12)') 'For wavelength: ',Mie_Inputs_Global(1)%lambda
           !stop 'at Fmatrix wvl check'
           ...
           call RTSMie_master_bimodal_OMP &
              ( Mie_Inputs_Global, Bimodal_fraction, & ! Inputs
                Mie_Outputs, Mie_fail, Mie_istatus, Mie_messages, Mie_trace_3 ) ! Outputs
           if ( Mie_fail ) go to 677

!  Write to file

            !write(44,'(/A,F12.5/)')' Mie Fmat data at wavelength ',Mie_Inputs_Global(1)%lambda
            !write(44,'(I5)')Mie_Outputs%Mie_NCoeffs
            do i = 1, Mie_Inputs_Global(1)%n_Fmatrix_angles
               write(44,'(I5,1p6e24.12)')i,Mie_Inputs_Global(1)%Fmatrix_angles(i),&
                                           Mie_Outputs%Mie_Fmatrix(1:4,i)/PI4
            enddo

           close(44)
           stop ' -- Finished Mie calculation for plotting '
         endif

!  First call for extinction coefficient at the Reference wavelength
!mick mod 12/9/2022 - replace std Mie call with Bimodal one using OpenMP
...
!mick mod 8/17/2023 - modified to display up to five pars
      ...
      write(*,  '(/A//4x,i2,36x,e10.2,5(1x,F13.8))')Trim(CHead), j, alamda, APARS(1:npars)
      write(U33,'(/A//4x,i2,36x,e10.2,5(1x,F13.8))')Trim(CHead), j, alamda, APARS(1:npars)

!  Do retrieval prep timing
...
!mick mod 9/12/2022 - added inputs DO_TIMING, NCORES
!                   - added inverse model timing
...
!mick mod 1/10/2023 - added output SIMJAC
         ...
         CALL TONGA_MRQMIN ( DO_TIMING, NCORES,                                                 & !Fwd model OMP inputs
                             J, E_MAXWAVS, MAXA, NDATA, MA, WAVS, MEASRADS, SIGS,               & !Fwd model Inputs
                             ...
                             OCHISQ, ATRY, BETA, DA, MFIT, SO2TAU, InvMod_Time, fail, messages, Mie_trace_3 ) !Inv model Output+Status
...
!mick debug
!write(*,*)
!write(*,*) 'chisq = ',chisq,' old_chisq = ',old_chisq,' rel chg = ',chisq_change
!do q = 1, MA
!   write(*,*) 'APARS(q) = ',APARS(q),' APARS_OLD(q) = ',APARS_OLD(q),&
!              'abs(rel chg) = ',abs((APARS(q)/APARS_OLD(q))-1.0_mpk),' par_conv = ',par_conv
!enddo
!write(*,*) 'npars = ',npars,' # pars converged = ',qc
...
!mick mod 9/12/2022  - added DO_TIMING & NCORES
!                    - added inverse model timing
!mick mod 9/22/2022  - enhanced error handling
!mick mod 12/9/2022  - To accomodate a Bimodal PSD, added "Bimodal_fraction" and "Mie_trace_3"
!mick mod 1/10/2023  - added output SIMJAC
...
!  Add inverse model timing

      if ( DO_TIMING ) then
         Call CPU_time(e1)
         Ret_Time = Ret_Time + InvMod_Time
      endif
...
!                     below where similar results are output (plus more)
      ...
      !   do w = 1, NDATA
      !      write(400,*)w,wavs(w),measrads(w),simrads(w)
      !   enddo
      !endif
...
!  Add retrieval clean-up timing / display timing for this retrieval pass
!mick mod 9/12/2022 - modified this section
!mick mod 5/9/2023  - added passing of ret timing to timing output file

      if ( DO_TIMING ) then
         call cpu_time(e2)
         Ret_Time = Ret_Time + e2-e1
         write(*,'(/1x,a,f6.2,a)')    'retrieval time = ',Ret_Time,' sec'
         write(U100,'(/1x,a,f6.2,a)') 'retrieval time = ',Ret_Time,' sec'
      endif
...
!  End Chi-Square Check Loop (this loop starts @ ~line 360)
