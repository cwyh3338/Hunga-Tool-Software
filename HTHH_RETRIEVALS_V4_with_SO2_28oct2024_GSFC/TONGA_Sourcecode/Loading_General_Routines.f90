Module Loading_General_Routines_m

public  :: profiles_uniform, profiles_expone, profiles_gdfone
private :: exp1_profile_only, exp1_profile_plus, gdf1_profile_only, gdf1_profile_plus
...
!  Height grid in [km]
      ...
      real(mpk),    intent(in) ::  heightgrid(0:maxlayers)
...
!    (2) Must have  z_l >/= heightgrid(nlayers). Also checked internally
      ...
      real(mpk),    intent(in) ::  z_upperlimit, z_lowerlimit

!  Total column between upper and lower limits

      real(mpk),    intent(in) ::  total_column
...
!  Umkehr profile
      ...
      real(mpk),    intent(out) ::  profile ( maxlayers )

!  Umkehr profile derivatives  (w.r.t. total column )

      real(mpk),    intent(out) :: d_profile_dcol ( maxlayers )
...
!  help variables
      ...
      real(mpk)        :: z1,z2,density, scaling, za,zb
...
!  Check input
      ...
      if ( z_lowerlimit .gt. z_upperlimit ) then
        fail = .true.
        message = 'Bad input:  Uniform slab lower limit > upper limit'
...
!    (w.r.t. total column and relaxation parameter)
      ...
      real(mpk),    intent(out) :: d_profile_dcol ( maxlayers )
      real(mpk),    intent(out) :: d_profile_drxn ( maxlayers )
...
!  help variables
      ...
      integer          :: n, nupper, nlower
      real(mpk)    :: k,b,ed,zd,kd,za,zb,term,omega,z1,z2,db_dk

!  Debug
!      logical          do_debug_profile
!      integer          m,msav,mdebug
!      parameter        ( mdebug = 1001 )
!      real(mpk)    pp(mdebug),zp(mdebug), sum, sumd, z, q
...
!  Arguments
      ...
      real(mpk),    intent(in)  :: za,zb,z1,b,k
      real(mpk),    intent(out) :: prof

!  Local variables

      real(mpk)    :: da,db,bok,ea,eb
...
!  Arguments
      ...
      real(mpk),    intent(in)  :: za,zb,z1,b,k,omega,db_dk
      real(mpk),    intent(out) :: prof,deriv1,deriv2

!  Local variables

      real(mpk)    :: da,db,ea,eb,bok,term1,term2,term3
...
!    (2) Must have  z_l > heightgrid(nlayers). Also checked internally
      ...
      real(mpk),    intent(in) :: z_upperlimit, z_lowerlimit,  z_peakheight

!  Half width

      real(mpk),    intent(in) :: half_width
...
!    (w.r.t. total column and peak height and half-width)
      ...
      real(mpk),    intent(out)  :: d_profile_dcol ( maxlayers )
      real(mpk),    intent(out)  :: d_profile_dpkh ( maxlayers )
      real(mpk),    intent(out)  :: d_profile_dhfw ( maxlayers )
...
!  help variables
      ...
      real(mpk)    :: omega,z1,z0,z2,h
      integer      :: n, nupper, nlower
      real(mpk)    :: a,zt1,zt2,za,zb
      real(mpk)    :: a_d,a_p,a_w,term,q1,q2,r1,r2,rsq1,rsq2
...
!      integer          m,msav,mdebug
!      parameter        ( mdebug = 1001 )
!      real(mpk)        pp(mdebug),zp(mdebug), sum, sumd, z, q, r
...
!  Check input
!mick mod 9/22/2022 - added 1st check ; adjusted 2nd-4th checks ; added comments

!  (1) Insure GDF envelope is contained within the atmosphere

      if ( z_upperlimit .gt. heightgrid(0) ) then
        fail = .true.
        message = 'Bad input: GDF Upper limit is above TOA'
        action  = 'decrease z_upperlimit < heightgrid(0)'
        return
      endif
...
!  (2) Insure GDF peak height is contained within the GDF envelope

      if ( z_peakheight .gt. z_upperlimit ) then
        fail = .true.
        message = 'Bad input: peak height above GDF Upper limit'
        action  = 'decrease z_peakheight < z_upperlimit'
        return
      endif

      if ( z_peakheight .lt. z_lowerlimit ) then
        fail = .true.
        message = 'Bad input: peak height below GDF Lower limit'
...
!  Arguments
      ...
      real(mpk),    intent(in)   :: za,zb,z0,h,a
      real(mpk),    intent(out)  :: prof
...
!  Arguments
      ...
      real(mpk),    intent(in)   :: za,zb,z0,h,a,a_d,a_p,a_w
      real(mpk),    intent(out)  :: prof,deriv1,deriv2,deriv3

!  Local variables

      real(mpk)    :: fac,qa,qb,ra,rb,mza,mzb,rsqa,rsqb,rab,fac_d
      real(mpk)    :: fac_p,ra_p,rb_p,rab_p
      real(mpk)    :: fac_w,ra_w,rb_w,rab_w
...
End Module Loading_General_Routines_m
